...
public class CharacterParser {
	...
	/**返回单词的所有拼音*/
	public List<String> convertEx(String str) {
		List<String> result=new ArrayList<String>();
		
		int ascii = getChsAscii(str);
		if (ascii > 0 && ascii < 160) {
			result.add(String.valueOf((char) ascii));
		} else {
			for (int i = (pyvalue.length - 1); i >= 0; i--) {
				if (pyvalue[i] <= ascii) {
					result.add( pystr[i]);					
	...
	public String getSellingHead(String chs) {
		...
		for (int i = 0; i < chs.length(); i++) {
			...
			if (key.getBytes().length >= 2) {
				//List<String> sList=convertEx(key);
				value = (String) convert(key);
				if (value == null) {
					value = "unknown";
				}
	 ...
	 * @return
	 */
	public String getSellingToNumber(String headStr){
		char[] ch=headStr.toCharArray();
		StringBuilder builder=new StringBuilder();
		for(int i=0,len=ch.length;i<len;i++){
			
			builder.append(getNumber(ch[i]));
		}
	...
	private String getNumber(char ch){
		...
		if(ch>=97&&ch<100)
			return "2";
		else if(ch>=100&&ch<103)
			return "3";
		else if(ch>=103&&ch<106)
			return "4";
		else if(ch>=106&&ch<109)
			return "5";
		else if(ch>=109&&ch<112)
			return "6";
		else if(ch>=112&&ch<116)
			return "7";
		else if(ch>=116&&ch<119)
			return "8";
